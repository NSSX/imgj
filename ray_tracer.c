...
t_vec3d	final_color(t_env *e)
{
  ...
  e->obj_tmp = all_inter(e);
	if (e->obj_tmp && e->value > 0.0001)
	{
	  ...
	  {
	    //	    e->color = (t_vec3d){255, 255, 255};
	    /*void    *mlx_xpm_to_image(void *mlx_ptr, char **xpm_data,
                          int *width, int *height);
void    *mlx_xpm_file_to_image(void *mlx_ptr, char *filename,
int *width, int *height);*/

	    //	    e->img->img_ptr = mlx_xpm_file_to_image(e->mlx, "ciel.xpm", &(e->width), &(e->height));
	    //	    e->color = (t_vec3d){0,0,0.3};
	    e->no = 1;
	    if(e->sky == 0)
	      {
		int a;
		...
		a = 0;
		b = 0;
		e->img->img_ptr = mlx_xpm_file_to_image(e->mlx, "ciel5.xpm", &(e->width), &(e->height));
		printf("%d\n", mlx_get_color_value(e->mlx, 0xFF00FF));
		e->sky = 1;
	      }
	  }
	return (e->color);
